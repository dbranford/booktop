...
impl Bookcase {
    ...
    pub fn get_books_by_keys<'k>(
        &'k self,
        keys: &'k [usize],
    ) -> impl Iterator<Item = Option<(&'k usize, &'k Book)>> {
        keys.iter().map(|k| self.get_book(k).map(|b| (k, b)))
    }
...
mod tests {
    ...
    fn get_books() {
        ...
        assert_eq!(
            b.get_books_by_keys(&[2, 1]).collect::<Vec<_>>(),
            vec![Some((&2, &test_book2())), Some((&1, &test_book1()))]
        );
